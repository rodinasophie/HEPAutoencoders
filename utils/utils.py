...
def normalized_reconstructions(
    model, unnormed_df, force_mean=None, force_std=None, idxs=None
):
    ...
    if idxs is not None:
        data = torch.FloatTensor(normed_df[idxs[0] : idxs[1]].values)
        unnormed_df = torch.FloatTensor(unnormed_df[idxs[0] : idxs[1]].values)
    else:
        data = torch.FloatTensor(normed_df.values)
        unnormed_df = torch.FloatTensor(unnormed_df.values)
...
def unnormalized_reconstructions(
    model, unnormed_df, force_mean=None, force_std=None, idxs=None
):
    if force_mean is None:
        mean = unnormed_df.mean()
...
def log_normalize(train, test=None):
    train["pT"] = train["pT"].apply(lambda x: np.log10(x) / 3.0)
    train["E"] = train["E"].apply(lambda x: np.log10(x) / 3.0)
    train["eta"] = train["eta"] / 3.0
    train["phi"] = train["phi"] / 3.0
    if test is not None:
        test["pT"] = test["pT"].apply(lambda x: np.log10(x) / 3.0)
        test["E"] = test["E"].apply(lambda x: np.log10(x) / 3.0)
        test["eta"] = test["eta"] / 3.0
        test["phi"] = test["phi"] / 3.0
...
def get_log_normalized_dls(train, test, bs=1024):
    ...
    train_ds = TensorDataset(
        torch.tensor(train_x.values, dtype=torch.float),
        torch.tensor(train_y.values, dtype=torch.float),
    )
    valid_ds = TensorDataset(
        torch.tensor(test_x.values, dtype=torch.float),
...
def filter_jets(train):
    train["pt"] = train["pt"] / 1000.0  # Convert to GeV
    train["m"] = train["m"] / 1000.0  # Convert to GeV
    train["LeadingClusterPt"] = train["LeadingClusterPt"] / 1000.0  # Convert to GeV
    train["LeadingClusterSecondR"] = (
        train["LeadingClusterSecondR"] / 1000.0
    )  # Convert to GeV
    train["LeadingClusterSecondLambda"] = (
        train["LeadingClusterSecondLambda"] / 1000.0
    )  # Convert to GeV
    train["NegativeE"] = train["NegativeE"] / 1000.0  # Convert to GeV
    ...
    train = train[
        np.invert((np.abs(train["EMFrac"]) < 0.05) & (np.abs(train["eta"]) >= 2))
    ]
    train = train[
        np.invert(
            (train["AverageLArQF"] > 0.8)
            & (train["EMFrac"] > 0.95)
            & (train["LArQuality"] > 0.8)
            & (np.abs(train["eta"]) < 2.8)
        )
    ]
    ...
    train = train[
        np.invert(
            (train["AverageLArQF"] > 0.8)
            & (np.abs(train["HECQuality"]) > 0.5)
            & (np.abs(train["HECFrac"]) > 0.5)
        )
    ]
...
def unit_convert_jets(leading, subleading):
    ...
    leading["pt"] = leading["pt"] / 1000.0  # Convert to GeV
    subleading["pt"] = subleading["pt"] / 1000.0  # Convert to GeV
    leading_orig["pt"] = leading_orig["pt"] / 1000.0  # Convert to GeV
    leading["m"] = leading["m"] / 1000.0  # Convert to GeV
    subleading["m"] = subleading["m"] / 1000.0  # Convert to GeV
    leading_orig["m"] = leading_orig["m"] / 1000.0  # Convert to GeV
    leading["LeadingClusterPt"] = leading["LeadingClusterPt"] / 1000.0  # Convert to GeV
    subleading["LeadingClusterPt"] = (
        subleading["LeadingClusterPt"] / 1000.0
    )  # Convert to GeV
    leading_orig["LeadingClusterPt"] = (
        leading_orig["LeadingClusterPt"] / 1000.0
    )  # Convert to GeV
    leading["LeadingClusterSecondR"] = (
        leading["LeadingClusterSecondR"] / 1000.0
    )  # Convert to GeV
    subleading["LeadingClusterSecondR"] = (
        subleading["LeadingClusterSecondR"] / 1000.0
    )  # Convert to GeV
    leading_orig["LeadingClusterSecondR"] = (
        leading_orig["LeadingClusterSecondR"] / 1000.0
    )  # Convert to GeV
    leading["LeadingClusterSecondLambda"] = (
        leading["LeadingClusterSecondLambda"] / 1000.0
    )  # Convert to GeV
    subleading["LeadingClusterSecondLambda"] = (
        subleading["LeadingClusterSecondLambda"] / 1000.0
    )  # Convert to GeV
    leading_orig["LeadingClusterSecondLambda"] = (
        leading_orig["LeadingClusterSecondLambda"] / 1000.0
    )  # Convert to GeV
    leading["NegativeE"] = leading["NegativeE"] / 1000.0  # Convert to GeV
    subleading["NegativeE"] = subleading["NegativeE"] / 1000.0  # Convert to GeV
    leading_orig["NegativeE"] = leading_orig["NegativeE"] / 1000.0  # Convert to GeV
...
def filter_mc_jets(leading, subleading):
    ...
    leading = leading[
        np.invert(
            (np.abs(leading_orig["EMFrac"]) < 0.05) & (np.abs(leading_orig["eta"]) >= 2)
        )
    ]
    ...
    leading = leading[
        np.invert(
            (leading_orig["AverageLArQF"] > 0.8)
            & (leading_orig["EMFrac"] > 0.95)
            & (leading_orig["LArQuality"] > 0.8)
            & (np.abs(leading_orig["eta"]) < 2.8)
        )
    ]
    subleading = subleading[
        np.invert(
    ...
    leading = leading[
        np.invert(
            (leading_orig["AverageLArQF"] > 0.8)
            & (np.abs(leading_orig["HECQuality"]) > 0.5)
            & (np.abs(leading_orig["HECFrac"]) > 0.5)
        )
    ]
    subleading = subleading[
        np.invert(
...
def custom_normalization(train, test):
    ...
    for data in [train_cp, test_cp]:
        ...
        data["ActiveArea4vec_m"] = (
            data["ActiveArea4vec_m"] / area4vecm_div - area4vecm_sub
        )
        ...
        data["LeadingClusterCenterLambda"] = (
            np.log10(data["LeadingClusterCenterLambda"] + log_add) - log_sub
        ) / centerlambda_div
        data["LeadingClusterSecondLambda"] = (
            np.log10(data["LeadingClusterSecondLambda"] + log_add) - log_sub
        ) / secondlambda_div
        data["LeadingClusterSecondR"] = (
            np.log10(data["LeadingClusterSecondR"] + log_add) - log_sub
        ) / secondR_div
        data["AverageLArQF"] = (
            np.log10(data["AverageLArQF"] + log_add) - log_sub
        ) / larqf_div
        data["pt"] = (np.log10(data["pt"]) - pt_sub) / pt_div
        data["LeadingClusterPt"] = np.log10(data["LeadingClusterPt"]) / pt_div
        data["CentroidR"] = (
            np.log10(data["CentroidR"]) - centroidR_sub
        ) / centroidR_div
...
def custom_unnormalize(normalized_data):
    ...
    data["ActiveArea"] = data["ActiveArea"] * area4vec_div
    data["ActiveArea4vec_m"] = (
        data["ActiveArea4vec_m"] + area4vecm_sub
    ) * area4vecm_div
    data["ActiveArea4vec_pt"] = data["ActiveArea4vec_pt"] * area4vecpt_div
    data["LArQuality"] = data["LArQuality"] * larq_div
    ...
    data["LeadingClusterCenterLambda"] = (
        np.power(10, centerlambda_div * data["LeadingClusterCenterLambda"] + log_sub)
        - log_add
    )
    data["LeadingClusterSecondLambda"] = (
        np.power(10, secondlambda_div * data["LeadingClusterSecondLambda"] + log_sub)
        - log_add
    )
    data["LeadingClusterSecondR"] = (
        np.power(10, secondR_div * data["LeadingClusterSecondR"] + log_sub) - log_add
    )
    data["AverageLArQF"] = (
        np.power(10, larqf_div * data["AverageLArQF"] + log_sub) - log_add
    )
